"""Read NDJSON inputs into row dicts."""

from flowline.core.errors import ReaderError


def read_ndjson(path, limit=None):
    """Load rows from a NDJSON file at *path*.
...
def _slurp(path):
    if not str(path).endswith(".ndjson"):
        raise ReaderError(f"expected a .ndjson file, got {path!r}")

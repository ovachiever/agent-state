"""Read SQLite dump inputs into row dicts."""

from flowline.core.errors import ReaderError


def read_sqlite_dump(path, limit=None):
    """Load rows from a SQLite dump file at *path*.
...
def _slurp(path):
    if not str(path).endswith(".sql"):
        raise ReaderError(f"expected a .sql file, got {path!r}")

...
class Scheduler:
    ...
    def __init__(self):
        ...
        self._staged = []     # entries created while a run is active
    ...
    def submit(self, fn, priority=0):
        ...
        self._tasks[tid] = {"fn": fn, "priority": priority, "state": "ready",
                            "order": order, "result": None}
        entry = (priority, order, tid)
        if self._running:
            # keep the heap stable while run() is draining it
        ...
        try:
            while self._heap:
                ...
                if task["state"] == "cancelled":
                    self._settle(tid, None)
                    continue
                result = task["fn"]()
                executed += 1
                if result == "again":
                    heapq.heappush(self._heap, (priority, order, tid))
                    continue
                self._settle(tid, result)
        finally:
            self._running = False
        return executed

    def _settle(self, tid, result):
        task = self._tasks[tid]
        task["result"] = result
        if task["state"] == "ready":
            task["state"] = "done"

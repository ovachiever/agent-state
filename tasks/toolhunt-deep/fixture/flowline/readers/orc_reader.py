"""Read ORC-lite inputs into row dicts."""

from flowline.core.errors import ReaderError


def read_orc(path, limit=None):
    """Load rows from a ORC-lite file at *path*.
...
def _slurp(path):
    if not str(path).endswith(".orc"):
        raise ReaderError(f"expected a .orc file, got {path!r}")

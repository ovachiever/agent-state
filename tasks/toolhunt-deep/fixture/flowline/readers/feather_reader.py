"""Read Feather-lite inputs into row dicts."""

from flowline.core.errors import ReaderError


def read_feather(path, limit=None):
    """Load rows from a Feather-lite file at *path*.
...
def _slurp(path):
    if not str(path).endswith(".feather"):
        raise ReaderError(f"expected a .feather file, got {path!r}")
